...
def Rand_Image():
    ...
    image = os.path.join(str("/static/"), str(rand_image))
    return image
#Rand_Image()

def Template_Loader():
    template_loader = FileSystemLoader(searchpath = "/bday_notification/")
...
def Send_Mail(emp_name, emp_email, message):
    ...
    try:
        ...
        smtp_obj.sendmail('', recipient, msg.as_string())
    except smtplib.SMTPException:
        print ("something went wrong")
#Send_Mail()

def Read_File():
    emp_data = '/bday_notification/emp_dob.json'
    message_data = '/bday_notification/message.json'
